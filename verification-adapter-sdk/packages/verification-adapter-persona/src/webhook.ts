...
async function hmacSha256Hex(crypto: Crypto, secret: string, value: Uint8Array): Promise<string> {
  ...
  return bytesToHex(new Uint8Array(await crypto.subtle.sign('HMAC', key, value as BufferSource)));
}

async function sha256HexBytes(crypto: Crypto, value: Uint8Array): Promise<string> {
